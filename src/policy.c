...
qd_error_t qd_entity_configure_policy(qd_policy_t *policy, qd_entity_t *entity)
{
    ...
    if (!module) {
        qd_log(LOG_POLICY, QD_LOG_CRITICAL,
...
void *qd_policy_c_counts_alloc(void)
{
        ...
        if (lookup_vhost_alias) {
            ...
            if (result) {
                ...
                if (res_string && res_len < name_buf_size) {
                    ...
                    qd_log(LOG_POLICY, QD_LOG_ERROR,
        ...
        if (lookup_user) {
            ...
            if (result) {
                ...
                if (res_string && res_len < name_buf_size) {
                    ...
                    qd_log(LOG_POLICY, QD_LOG_ERROR, "Internal: lookup_user: insufficient buffer for name");
                }
                Py_XDECREF(result);
                free(res_string);
                res = !!name_buf[0]; // settings name returned
...
bool qd_policy_host_pattern_add(qd_policy_t *policy, const char *hostPattern)
{
    ...
    if (rc != QD_ERROR_NONE) {
        ...
        free(payload);
        qd_log(LOG_POLICY, QD_LOG_WARNING, "vhost hostname pattern '%s' add failed: %s", hostPattern,
...
void qd_policy_host_pattern_remove(qd_policy_t *policy, const char *hostPattern)
{
    ...
    if (oldp) {
        ...
        qd_log(LOG_POLICY, QD_LOG_WARNING, "vhost hostname pattern '%s' for removal not found", hostPattern);
    }
}
